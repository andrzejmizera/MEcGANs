...
def save_images(input_image, output_image, results_dir, current_n):
    for i, (x, y) in enumerate(zip(input_image, output_image)):
        ...
        if x.shape[2] == 1:
            ...
            cv2.imwrite(os.path.join(results_dir, '{:03d}_input_sar_v1.png'.format(current_n + i)), x[:, :, 0])
            cv2.imwrite(os.path.join(results_dir, '{:03d}_input_sar_v2.png'.format(current_n + i)), x[:, :, 1])
            cv2.imwrite(os.path.join(results_dir, '{:03d}_input_rgb.png'.format(current_n + i)), x[:, :, 2:])
        elif x.shape[2] == 15:
            cv2.imwrite(os.path.join(results_dir, '{:03d}_input_sar_v1.png'.format(current_n + i)), x[:, :, 0])
            cv2.imwrite(os.path.join(results_dir, '{:03d}_input_sar_v2.png'.format(current_n + i)), x[:, :, 1])
            cv2.imwrite(os.path.join(results_dir, '{:03d}_input_rgb.png'.format(current_n + i)), x[:, :, 5:2:-1])
        else:
