...
class ConcatDiscriminator(chainer.Chain):
    def __init__(self, in_ch, out_ch, in_edges):
        layers = {}
        w = chainer.initializers.Normal(0.02)
        ...
        layers['c0_e'] = CBR(in_edges, 1, bn=False, sample='down', activation=F.leaky_relu, dropout=False)
        layers['c1'] = CBR(65, 128, bn=True, sample='down', activation=F.leaky_relu, dropout=False)
    ...
    def __call__(self, x_0, x_1, x_edges):
        h = F.concat([self.c0_0(x_0), self.c0_1(x_1), self.c0_e(x_edges)])
        h = self.c1(h)
        h = self.c2(h)
        h = self.c3(h)
        h = self.c4(h)
