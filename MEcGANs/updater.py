...
class Updater(chainer.training.StandardUpdater):
    ...
    def get_edges_batch(self, xp, rgbcloud_batch):
        #batchsize = len(rgbcloud_batch)

        dims = rgbcloud_batch.shape
        batchsize = dims[0]

        edges_batch = xp.ndarray([dims[0],1,dims[2],dims[3]],dtype=np.dtype(np.float32))
        for j in range(batchsize):
            ...
            #edges_batch[j,:,:,:] = xp.concatenate((img,edges[None,:,:]),axis=0)
            edges_batch[j,:,:,:] = edges[None,:,:]
    ...
    def update_core(self):
        ...
        y_edges = self.get_edges_batch(xp, y)
        
        for i in range(self.n_dis):
            if i == 0:
                y_fake = gen(x)
                
                y_fake_edges = self.get_edges_batch(xp, y_fake)

                dis_fake = dis(x, y_fake, y_fake_edges)
                loss_gen = self.loss_gen(dis_fake=dis_fake)

                #g = c.build_computational_graph(loss_gen)
                #with open('comp_graph_mecgans_corr.gv', 'w') as o:
                #    o.write(g.dump())
                #exit()
